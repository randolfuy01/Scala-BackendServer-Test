...
object CassandraConnection {
  ...
  def messaging(chat: UUID, sender: String, message: String): Int = {
    try {
      val query =
        """INSERT INTO chat_app.message (messageid, senderid, chatroom, message, date_posted)
        |VALUES (?, ?, ?, ?, ?);""".stripMargin
      ...
      case e: Exception =>
        e.printStackTrace()
        println(s"Failed to insert message into database: ${e.getMessage}")
        500
    }
  }
}
